...
COMPARTMENT_OCID="ocid1.compartment.oc1..aaaaaaaacufz3wn5ugdwaglqsmkbpzt4szgiiglhd672hpgppgb5ixnc4zma"
...
say "Stop previous Nitu Travels gateway compute instances in project compartment only"
...
done < <(oci compute instance list --compartment-id "$COMPARTMENT_OCID" --all 2>/dev/null \
...
RESERVED_IP_ID="$(oci network public-ip list --scope REGION --compartment-id "$COMPARTMENT_OCID" --all 2>/dev/null \
