...
# Keep the tested deployment payload and replace Ed25519, which Oracle Cloud Shell
# rejects when its OpenSSL/OpenSSH environment is operating in FIPS mode.
sed -i "s/01a27dc20b5b104dfea74f62179c05e87a34a3c0/${SUPPORT_REF}/g" "$TMP"
sed -i "s/ssh-keygen -q -t ed25519 -N '' -f \"\$KEY\"/ssh-keygen -q -t rsa -b 3072 -N '' -f \"\$KEY\"/" "$TMP"
